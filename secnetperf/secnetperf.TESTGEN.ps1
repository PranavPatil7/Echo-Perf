...
function test_recv {
    ...
    [string] $cmd = "./secnetperf -port:$port -exec:$Exec"
...
function test_send_throughput {
    ...
    [string] $cmd = "./secnetperf -target:$DestIp -port:$Port -tcp:$Type -iosize:$Len -conns:$Conns -up:$($Secs)s -down:$($Secs)s -exec:maxtput -ptput:1 $Options > $out.txt"
    [string] $cmdOut = (Join-Path -Path $OutDir -ChildPath "$Fname")
    Write-Output $cmd | Out-File -Encoding ascii -Append "$cmdOut.txt"
    Write-Output $cmd | Out-File -Encoding ascii -Append $g_log
    Write-Output $cmd | Out-File -Encoding ascii -Append $g_logSend
...
function test_latency {
    ...
    for ($i=0; $i -lt $g_Config.Iterations; $i++) {
        ...
        test_send_latency -Port $portstart -Secs $Config.Runtime -Size $Config.ByteSize -Type $protoParam -Requests $Config.Requests -OutDir $dir -Fname "$Proto.send.$Fname.iter$i" -Options $Options -SendDir $send_dir
    }
}
...
function test_handshakes {
    ...
    for ($j=0; $j -lt $Config.Connections.Length; $j++) {
        ...
        for ($i=0; $i -lt $g_Config.Iterations; $i++) {
            ...
            test_send_handshakes -Port $portstart -Secs $Config.Runtime -Conns $Conn -Type $protoParam -OutDir $dir -Fname "$Proto.send.$Fname.iter$i" -Options $Options -SendDir $send_dir
        }
    }
}
    ...
    for ($j=0; $j -lt $Config.Connections.Length; $j++) {
        ...
        for ($k=0; $k -lt $Config.BufferLen.Length; $k++) {
            ...
            for ($i=0; $i -lt $g_Config.Iterations; $i++) {
                ...
                [int] $portstart = $g_Config.StartPort + ($i * $g_Config.Iterations)
                test_recv -Port $portstart -Exec 'maxtput'
                test_send_throughput -Port $portstart -Secs $Config.Runtime -Conns $Conn -Len $Len -Type $protoParam -OutDir $dir -Fname "$Proto.send.$Fname.iter$i" -Options $Options -SendDir $send_dir
            }
        }
    }
}
...
function test_secnetperf_generate {
    ...
    if ($null -ne $g_Config.TestType.Handshakes) {
        ...
        if ($null -ne $g_Config.TestType.Handshakes.tcp) {
            test_handshakes -OutDir $dir -Proto 'tcp' -SendDir $send_dir
        }
        if ($null -ne $g_Config.TestType.Handshakes.quic) {
            test_handshakes -OutDir $dir -Proto 'quic' -SendDir $send_dir
        }
    }
    ...
    if ($null -ne $g_Config.TestType.Throughput) {
        ...
        if ($null -ne $g_Config.TestType.Throughput.quic) {
            test_throughput -OutDir $dir -Proto 'quic' -SendDir $send_dir 
        }
    }
} # test_secnetperf()
...
function test_main {
    ...
    try {
        ...
        [string] $sendDir   = (Join-Path -Path $SrcDir -ChildPath "secnetperf")
        ...
        test_secnetperf_generate -OutDir $dir -SendDir $sendDir 
    } catch {
